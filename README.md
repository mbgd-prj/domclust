...
This compiles the program; the executable is produced as `bin/domclust`.
You can test the program by typing `make test`, if Perl is installed
on your computer.
...
To install the program into a standard location (the default is
`/usr/local/bin`), run:
...
To install into a different location, pass `--prefix` to `configure`
before building, e.g.:
...
This installs the executable as `$HOME/local/bin/domclust`. Use
`make uninstall` to remove it again.

`configure` also accepts the following options to build development
variants of the program; these are intended for development/special-purpose
use and are not needed for normal use:

- `--enable-dev` — build with gene-neighbor / domain-cut / new-alignment support
- `--enable-large` — enable large-scale mode with nucleotide reverse-match direction tracking
...
You can specify an output format by the `-o` option. Possibly useful
...
### C. Newick format without branch lengths (`-o2`), or with branch lengths (`-o3`)
...
Example session:
```sh
bin/domclust tst/test.hom tst/test.gene -o6 | agrep -dCluster bsu:MTLA | \
	Script/convgraph.pl | dotty -
```

[ To execute this, you need also the `agrep` command included in the
Glimpse package as well as the GraphViz package. ]
...
information by the `-R<dump_file>` option. This option is particularly
useful when you want to execute the program several times while
changing the parameters that affect only the post-processing
phase including output format; such parameters are `-n`, `-ne`, `-p`, `-H`,
`-ai`, `-ao`, `-t` and `-o` among the above options.

Example session:
```sh
bin/domclust homfile genefile -o10 > dumpfile
bin/domclust -Rdumpfile > result1
bin/domclust -Rdumpfile -p0.6 -o1 > result2
```
