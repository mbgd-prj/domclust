...
/*
#define SIZE_INCR_RATIO 1.25
*/
#define SIZE_INCR_RATIO 1.6
...
setArray(varArray *vararray, int idx, char *datum)
{
	while (idx >= vararray->allocsize) {
		reallocArray(vararray, (int) (vararray->allocsize * SIZE_INCR_RATIO));
	}
...
reallocArray(varArray *vararray, int size)
{
	if ((vararray->array = realloc(vararray->array,
		...
		fprintf(stderr, "Can't realloc vararray: %d\n", size*vararray->datasize);
		exit(1);
	}
...
char *getRawArray(varArray *array)
{
	return array->array;
}
