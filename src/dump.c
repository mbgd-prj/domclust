...
#elif defined LARGE
#define NUM_EDGE_FLD 11
#else
#define NUM_EDGE_FLD 10
#endif
...
dumpEdge1(FILE *ofp, Edge *edge)
{
	...
	fprintf(ofp, " %d ", edge->flag);
#ifdef LARGE
	fprintf(ofp, " %d",  edge->dir);
#endif
#ifdef EXPERIMENTAL
	fprintf(ofp, " %d",  edge->connect);
#endif
	fprintf(ofp, "\n");
}
#ifdef WITH_NEIGHBOR
...
dumpEdge2(Edge *edge)
{
	if (edge->left) dumpEdgeNeighborList(edge->left, "NEL");
	if (edge->right) dumpEdgeNeighborList(edge->right, "NER");
}
*/
#endif
		...
		int curr_argc, char **curr_argv)
{
	...
	while (fgets(buf, BUFSIZ, fp) != NULL) {
		...
		if (buf[0] == '#') {
			...
			if (buf[0] == 'G') {
				sscanf(buf, "G %d%d%d", &nodenum,&leafnum,
							&edgenum);
