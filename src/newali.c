	...
	char *hitflag, pList *newEdges)
{
	...
	ConnCount newconn;
	signed char edir1 = 0, edir2 = 0, newedir = 0;
	Count cnt1 = n1->cnt;
	Count cnt2 = n2->cnt;
	Dist score1, score2, dist1, dist2;
	ConnCount conn1 = 0, conn2 = 0;
	Edge *newedge;
	int dir;
...
*/
	...
	/** update the distance and the number of connections **/
		...
		if (Opt.revMatch) {
			...
			if (edir1 && edir2) {
				/* pick the direction of the edge with a larger score */
				if (score1 > score2) {
					newedir = edir1;
				} else {
					newedir = best->dir * edir2;
		...
		Dist *newdist, Dist *newscore, ConnCount *newconn)
{
	...
	*newscore =(Dist) (score1 * cnt1 * flag1 + score2 * cnt2 * flag2)
			/ (Dist) (cnt1 * flag1 + cnt2 * flag2);
#ifdef EXPERIMENTAL
	*newconn = ((int) (conn1 + conn2) > MAX_CONN)
			? MAX_CONN : conn1 + conn2;
#endif
}
